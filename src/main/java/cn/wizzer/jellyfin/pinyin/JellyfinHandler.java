    ...
    public void renderItems(List<NutMap> items) {
        ...
        for (NutMap item : items) {
            ...
            if (folders.contains(item.getString("Type"))) {
                ...
                String forcedSortName = itemDetail.getString("ForcedSortName");
                String pinyin = itemDetail.getString("Name") == null ? "" : PinyinUtil.getPingYin(itemDetail.getString("Name"));
                if (Strings.isNotBlank(pinyin) && pinyin.length() > 50) {
                    pinyin = pinyin.substring(0, 50);
                }
                if (Strings.isNotBlank(forcedSortName) && pinyin.equals(forcedSortName)) {
                    log.infof("跳过 %s", itemDetail.getString("Name"));
                    this.skipCount++;
                } else {
                    log.infof("处理 %s", itemDetail.getString("Name"));
